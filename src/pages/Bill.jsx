...
export default function Bill({
  ...
  const total = cartItems.reduce((sum, item) => {
    const price = parseFloat(
      item.price?.replace("₹", "").replace(/,/g, "") || 0,
    );
    return sum + price;
  }, 0);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          <div className="bg-white px-4 py-3 rounded-xl shadow-sm border border-slate-200 flex items-center gap-3">
            ...
            <p className="text-sm font-medium text-slate-600">
              Order sent to WhatsApp automatically.
            </p>
          </div>
        ...
        <motion.div
          ...
          style={{
            ...
            minHeight: "297mm",
            margin: "0 auto",
          }} // A4 dimensions approximation
        >
          <div
            ref={billRef}
            className="p-8 md:p-12 h-full flex flex-col relative"
          >
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-slate-100 pb-8 mb-8 relative z-10 gap-6 md:gap-0">
              ...
              <div className="text-left md:text-right flex flex-col items-start md:items-end w-full md:w-auto">
                ...
                <p className="text-sm text-slate-600">
                  Ph: {companyData.mobile}
                </p>
                <p className="text-sm text-slate-600 text-blue-600 font-bold">
                  {companyData.website}
                </p>
              </div>
            </header>
              ...
              <table className="w-full mb-8">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="py-3 px-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider w-24">
                      Qty
                    </th>
                    <th className="py-3 px-4 text-right text-xs font-bold text-slate-500 uppercase tracking-wider w-32">
                      Price
                    </th>
                  </tr>
                </thead>
            ...
            <div className="bg-slate-50 rounded-lg p-4 mt-8 text-center md:text-left">
              <p className="text-xs text-slate-500 leading-relaxed">
                <strong>Terms & Conditions:</strong> Goods once sold will not be
                taken back. Warranty as per manufacturer terms. This is a
